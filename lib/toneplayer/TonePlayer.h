...
#endif   // USE_AVR_TIMER1
  void setSong(const int8_t * ptr_song_buffer, uint16_t tone_size, uint16_t bpm = DEFAULT_BPM);
...
private:
  ...
  int8_t  stackPointer, toneCode;
  const int8_t * songBuffer;
  int8_t   toneShift;
...
void TonePlayer::setSong(const int8_t* ptr_song_buffer, uint16_t tone_size, uint16_t bpm) {
  if (isPlaying) {
    stop();
  }
